...
open ball, center $x^\star$, radius $\epsilon$
...
$\mathcal{C}$ closed
$\Rightarrow \exists$
minimizer of (\ref{eq:optimization}) described by:
$$
\min_{x \in \mathcal{C}} f(x) \text{\ and\ } \underset{x \in
\mathcal{C}}{\operatorname{argmin}} f(x) % todo check argmin
$$
\end{proposition}
...
s.t. achieving
$|f(x_N )−f(x^\star)| < \epsilon $
requires
\[
N \ge
\left(\left\lfloor
\frac{L}{2\epsilon}
\right\rfloor\right)^n-1
\]
\end{proposition}
...
raster 1/3, 9 boxes in (1,1), for $N\le 7$ (8 steps) one grid cell
is not visited
...
- Partition unit cube into $s^n$ small boxes with side length $1/s$
and $min_xinC = -L/2s$
...
$$
q(\theta x+(1 − \theta)y) \le \theta q(x) + (1 −
\theta)q(y)\quad\forall\ \theta \in [0, 1]
$$
\end{definition}
